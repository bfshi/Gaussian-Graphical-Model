function [T_save,d_save,C_save,Sig_save,lambda_save] = Bglasso_mcmc_chol_lambdaunknown(S,n,a_lambda,b_lambda,C,burnin,nmc)
%Input:
        ...
        [boundary_incr,bb] = sort(boundary);
           ...
           if(k==0)
             ...
             if( 1<ii & ii <length(boundary)+1)    
               ...
               upper = boundary_incr(ii);
%               D(j,j) = sample_gamrnd_truncated(shape_post,1/scale_post(ii),lower,upper);
            switch sign(scale_post(ii))
                   case 1
               D(j,j) = sample_gamrnd_truncated(shape_post,1/scale_post(ii),lower,upper);
                   case -1
               D(j,j) = sample_pexp_truncated_twosided(shape_post,-scale_post(ii),lower,upper);
            end
...
%               D(j,j) =    sample_gamrnd_truncated_above(shape_post,1/scale_post(ii),upper);
               ...
               D(j,j) = sample_gamrnd_truncated_above(shape_post,1/scale_post(ii),upper);
                    case -1
               D(j,j) = sample_pexp_truncated_above(shape_post,-scale_post(ii),upper);
                 end 
...
%%%% L1 shrinkage %%%%%%%%%%%%%%%%%%%%%%%%%%        
     ...
     w = log([w(1:end,1);1]-[0;w(1:end,2)]) -c2+1/2*(m1+m2).^2./pre_T;           
      ...
      w = (exp(w-max(w)))./sum((exp(w-max(w))));
           ...
           lower = boundary_incr(ii-1);
           upper = boundary_incr(ii);
           T(i,j) = rand_truncated_normal_rej_twosided(mu_T(ii),1/sqrt(pre_T),lower,upper);
  
           
           elseif(ii == 1);
           upper = boundary_incr(ii);
           T(i,j) = rand_truncated_normal_rej_above(mu_T(ii),1/sqrt(pre_T),upper);%rand_tn_above(mu_T(ii),1/sqrt(pre_T),upper);

           
           else
           lower = boundary_incr(ii-1);
           T(i,j) = rand_truncated_normal_rej_below(mu_T(ii),1/sqrt(pre_T),lower);
           end
