function [T_save,d_save,C_save,Sig_save] = Bglasso_mcmc_chol_lambdafix(S,n,lambda,C,burnin,nmc)
%Input:
...
%%%% L1 shrinkage %%%%%%%%%%%%%%%%%%%%%%%%%%        
     ...
     w = log([w(1:end,1);1]-[0;w(1:end,2)]) -c2+1/2*(m1+m2).^2./pre_T;           
      ...
      w = (exp(w-max(w)))./sum((exp(w-max(w))));
           ...
           elseif(ii == 1);
           upper = boundary_incr(ii);
           T(i,j) = rand_truncated_normal_rej_above(mu_T(ii),1/sqrt(pre_T),upper);
                 if(T(i,j) == Inf)
                    return;
                end
